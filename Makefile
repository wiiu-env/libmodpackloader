...
#---------------------------------------------------------------------------------
all: lib/libmodpackloader.a

dist-bin: all
	@tar --exclude=*~ -cjf libmodpackloader-$(VERSION).tar.bz2 include lib

dist-src:
	@tar --exclude=*~ -cjf libmodpackloader-src-$(VERSION).tar.bz2 include source Makefile

dist: dist-src dist-bin

install: dist-bin
	mkdir -p $(DESTDIR)$(DEVKITPRO)/wums
	bzip2 -cd libmodpackloader-$(VERSION).tar.bz2 | tar -xf - -C $(DESTDIR)$(DEVKITPRO)/wums
...
lib/libmodpackloader.a :$(SOURCES) $(INCLUDES) | lib release
